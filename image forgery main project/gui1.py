...
from testimage import predict_result
import time
...
def showcheck():
    ...
    b3=Button(f4,text="Prediction",font="Verdana 10 bold",command=lambda:process2(path1,lb1))
    b3.pack(pady=2)
...
def process2(path2,lb1):
    ...
    global f6,f2,top
    prediction = predict_result(path2)    
    ...
    lb1.after(10,delayed_insert,lb1,0,'Read image')
    top.update()
    lb1.after(10,delayed_insert,lb1,0,'Preprocessing Started ')
    top.update()
    lb1.after(10,delayed_insert,lb1,0,'Features Extraction')
    top.update()
    lb1.after(10,delayed_insert,lb1,0,'Model Loaded')
    top.update()
    lb1.after(10,delayed_insert,lb1,0,'Predicted Successfully ')
    top.update()
    ...
    lb2.after(10,showresult,prediction)    
...
def delayed_display():
    global f6,f2
    image = Image.open("ela_image.png")
    photo = ImageTk.PhotoImage(image.resize((500, 350), Image.ANTIALIAS))
    label = Label(f6, image=photo, bg=main_color)
    label.image = photo
    label.pack()
...
if __name__=="__main__":
    ...
    image = image.resize((1500, 700), Image.ANTIALIAS) 
    pic = ImageTk.PhotoImage(image)
    lbl_reg=Label(f,image=pic,anchor=CENTER)
